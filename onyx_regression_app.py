...
def main_page():
    ...
    with ui.card().classes('absolute-center').style('width: 500px; padding: 30px; background-color: #f0f0f0'):
        ...
        with ui.button('Simple Linear Regression', on_click=lambda: ui.navigate.to('/simple')).classes('w-full').style(
            'height: 80px; background-color: #4CAF50; color: white; font-size: 14px; margin: 10px 0'
        ):
            ui.label('(Study Hours → Marks)').style('font-size: 12px; margin-top: 5px')
        
        with ui.button('Polynomial Regression', on_click=lambda: ui.navigate.to('/polynomial')).classes('w-full').style(
            'height: 80px; background-color: #2196F3; color: white; font-size: 14px; margin: 10px 0'
        ):
            ui.label('(Level → Salary)').style('font-size: 12px; margin-top: 5px')
        
        with ui.button('Multiple Linear Regression', on_click=lambda: ui.navigate.to('/multiple')).classes('w-full').style(
            'height: 80px; background-color: #FF9800; color: white; font-size: 14px; margin: 10px 0'
        ):
            ui.label('(Startup Profit Prediction)').style('font-size: 12px; margin-top: 5px')
...
def simple_page():
    ...
    with ui.card().classes('absolute-center').style('width: 450px; padding: 30px; background-color: #f0f0f0'):
        ...
        with ui.row().classes('w-full items-center'):
            ui.label('Study Hours (1-10):').style('width: 150px; font-size: 14px')
            hours_input = ui.number(value=5, min=1, max=10, step=0.5).style('width: 150px')
        
        result_label = ui.label('').classes('text-h6 text-weight-bold text-center').style('color: #4CAF50; margin: 20px 0; min-height: 30px')
        ...
        def predict():
            try:
                ...
                if hrs >= 1 and hrs <= 10:
                    ...
                    result_label.set_text(f'Predicted Marks: {int(marks[0])}')
                    result_label.style('color: #4CAF50')
                else:
                    result_label.set_text('Please enter hours between 1 and 10')
                    result_label.style('color: #f44336')
            except Exception as e:
                result_label.set_text(f'Error: {str(e)}')
                result_label.style('color: #f44336')
        
        ui.button('Predict', on_click=predict).classes('w-full').style('background-color: #4CAF50; color: white; margin: 10px 0; height: 45px; font-size: 16px')
        ui.button('Back to Menu', on_click=lambda: ui.navigate.to('/')).classes('w-full').style('background-color: #757575; color: white; height: 40px')
...
def polynomial_page():
    ...
    with ui.card().classes('absolute-center').style('width: 450px; padding: 30px; background-color: #f0f0f0'):
        ...
        with ui.row().classes('w-full items-center'):
            ui.label('Level:').style('width: 150px; font-size: 14px')
            level_input = ui.number(value=5, min=1, max=10, step=1, format='%.0f').style('width: 150px')
        
        result_label = ui.label('').classes('text-h6 text-weight-bold text-center').style('color: #2196F3; margin: 20px 0; min-height: 30px')
        ...
        def predict():
            try:
                ...
                result_label.set_text(f'Predicted Salary: ${int(predict_sal[0]):,}')
                result_label.style('color: #2196F3')
            except Exception as e:
                result_label.set_text(f'Error: {str(e)}')
                result_label.style('color: #f44336')
        
        ui.button('Predict', on_click=predict).classes('w-full').style('background-color: #2196F3; color: white; margin: 10px 0; height: 45px; font-size: 16px')
        ui.button('Back to Menu', on_click=lambda: ui.navigate.to('/')).classes('w-full').style('background-color: #757575; color: white; height: 40px')
...
def multiple_page():
    ...
    with ui.card().classes('absolute-center').style('width: 500px; padding: 30px; background-color: #f0f0f0'):
        ...
        result_label = ui.label('').classes('text-h6 text-weight-bold text-center').style('color: #FF9800; margin: 20px 0; min-height: 30px')
        ...
        def predict():
            try:
                ...
                if california not in [0, 1] or newyork not in [0, 1] or florida not in [0, 1]:
                    result_label.set_text('Location values must be 0 or 1')
                    result_label.style('color: #f44336')
                    return
                ...
                result_label.set_text(f'Predicted Profit: ${int(prediction[0]):,}')
                result_label.style('color: #FF9800')
            except Exception as e:
                result_label.set_text(f'Error: {str(e)}')
                result_label.style('color: #f44336')
        
        ui.button('Predict', on_click=predict).classes('w-full').style('background-color: #FF9800; color: white; margin: 10px 0; height: 45px; font-size: 16px')
        ui.button('Back to Menu', on_click=lambda: ui.navigate.to('/')).classes('w-full').style('background-color: #757575; color: white; height: 40px')
